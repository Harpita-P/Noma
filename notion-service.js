...
class NotionService {
  ...
  /**
   * Initialize the service with an integration token
   ...
   * @param {string} pageUrlOrId - Notion page URL or ID
   * @returns {string} - Clean page ID
   */
   ...
   * @param {string} pageId - Page ID without dashes
   * @returns {string} - Formatted page ID
   */
  formatPageId(pageId) {
   ...
   * @param {string} blockId - Block ID to get children from
   */
  async getBlockChildren(blockId) {
    ...
    while (hasMore) {
      ...
      if (!response.ok) {
        return allBlocks; // Return what we have if we can't get children
      }
  ...
  async extractBlockText(block) {
    ...
    switch (type) {
      ...
      case 'table_row':
        ...
        break;
      default:
        // For other types, try to extract any rich_text if available
        if (block[type] && block[type].rich_text) {
          text = this.extractRichText(block[type].rich_text);
        }
    }

    // Get nested blocks if they exist
   ...
   * @returns {Object} - Object containing page title and full text content
   */
