(function () {
  ...
  async function createSessionWithDownload(expected, {
    systemPrompt
  }
  = {}) {
      const LM = getLM();
  ...
  async function runMultimodalPrompt(contextData, userPrompt, abortSignal) {
    ...
    async function getContexts(tagId) {
      ...
      = await chrome.storage.local.get(CTX_KEY);
            ...
            try {
                const {
          'noma-calendar-tags': calendarTags = {}, 'noma-calendar-contexts': calendarContexts = {}
        }
        = 
          await chrome.storage.local.get(['noma-calendar-tags', 'noma-calendar-contexts']);
            ...
            try {
                const {
          'noma-gmail-tags': gmailTags = {}, 'noma-gmail-contexts': gmailContexts = {}
        }
        = 
          await chrome.storage.local.get(['noma-gmail-tags', 'noma-gmail-contexts']);
            ...
            try {
                const {
          'noma-notion-tags': notionTags = {}, 'noma-notion-contexts': notionContexts = {}
        }
        = 
          await chrome.storage.local.get(['noma-notion-tags', 'noma-notion-contexts']);
            ...
            try {
                const {
          'noma-pinterest-tags': pinterestTags = {}, 'noma-pinterest-contexts': pinterestContexts = {}
        }
        = 
      ...
      = await chrome.storage.local.get(CTX_KEY);
        ...
        total: contexts.length
            ...
            if (error.message.includes('Extension context invalidated')) {
                ...
                return {
          text: 0, pdf: 0, image: 0, calendar: 0, email: 0, notion: 0, audio: 0, total: 0
        };
      }
            throw error;
    }
    ...
    async function getAllTagsWithContextCounts() {
      ...
      = await chrome.storage.local.get(TAGS_KEY);
            ...
            try {
                const {
          'noma-calendar-tags': storedCalendarTags = {}
        }
        = await chrome.storage.local.get('noma-calendar-tags');
            ...
            try {
                const {
          'noma-gmail-tags': storedGmailTags = {}
        }
        = await chrome.storage.local.get('noma-gmail-tags');
            ...
            try {
                const {
          'noma-notion-tags': storedNotionTags = {}
        }
        = await chrome.storage.local.get('noma-notion-tags');
            ...
            try {
                const {
          'noma-pinterest-tags': storedPinterestTags = {}
        }
    ...
    async function buildContextData(tagId, {
    maxChars = 100000
  }
  = {}) {
        try {
                ...
                if (item.type === "image" && item.imageData) {
                    const imagePromise = (async () => {
                        try {
                            ...
                            return new File([blob], `image-${item.id}`, {
                type: item.mimeType || 'image/jpeg'
                    ...
                    const imagePromise = (async () => {
                        try {
                            ...
                            const blob = await response.blob();
                            return new File([blob], `pinterest-${item.id}`, {
                type: 'image/jpeg'
              });
            }
            catch (e) {
        ...
        totalChars: ragAnalysis?.totalChars || textBlob.length
            ...
            if (error.message.includes('Extension context invalidated')) {
                ...
                return {
          textBlob: "", images: [], ragAnalysis: null, isLargeContext: false
        };
      }
            throw error;
    }
    ...
    contextText, userPrompt, liveContextText
  }) {
    ...
    async function showTagSelector(element) {
      ...
      storedCaretPosition: storedCaretPosition
        ...
        console.log('Taggle: Dynamic tags:', dynamicTags.map(t => ({
      name: t.name, isCalendar: t.isCalendarTag, isGmail: t.isGmailTag, isNotion: t.isNotionTag, isPinterest: t.isPinterestTag
    })));
        ...
        text: 0, pdf: 0, image: 0, calendar: 0, email: 0, notion: 0, pinterest: 0, total: 0
      };
    ...
    document.addEventListener('keydown', (e) => {
        console.log("Taggle: CAPTURE - Key pressed:", e.key, "Code:", e.code, "Ctrl:", e.ctrlKey, "Meta:", e.metaKey, "KeyCode:", e.keyCode);
      ...
      document.addEventListener('keydown', (e) => {
        console.log("Taggle: Key pressed:", e.key, "Code:", e.code, "Ctrl:", e.ctrlKey, "Meta:", e.metaKey, "KeyCode:", e.keyCode);
        if ((e.key === 'q' || e.key === 'Q' || e.code === 'KeyQ') && (e.ctrlKey || e.metaKey)) {
        ...
        `.replace(/\s+/g, ' ').trim());
                if (fallbackElement) {
                    ...
                    console.log("Taggle: No editable elements found on page");
                    toast("Please click on a text input field first, then press Ctrl+Q");
        }
      }
    }
